...
## Wrapping lines (`WrapChildren`)

`Style.WrapChildren = true` moves children that no longer fit onto a new line. Each line is laid
out **on its own**: `Grow` stretches elements within their line and `JustifyContent` distributes
them there — otherwise stretching would eat the whole container and wrapping would lose its point.
...
When children do not fit, they shrink in proportion to their size and to the `Style.Shrink` weight
(the counterpart of CSS `flex-shrink`). `Shrink = 0` means "do not shrink me": the element keeps
its size and the others take up the shortfall. If nothing can shrink, the content honestly goes
past the border, and from there `Overflow` decides what happens.

**`Stretch` does not override an explicit size.** `AlignItems.Stretch` stretches a child along the
cross axis only when the size along that axis is not set — just like `align-items: stretch` in
CSS. A bar with `Width = 300` inside a column stays 300 wide instead of filling the full width.
