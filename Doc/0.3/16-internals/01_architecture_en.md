...
A click, the wheel and the cursor go to **exactly one** element — the first that handled it. In
frame order that turns out to be the deepest element under the mouse: containers emit their
children before their own handlers.

| Event | Rule |
|---|---|
| Click | Taken by the first handler. The element beneath never sees the press. |
| Wheel | Goes to whatever is under the cursor; scrolling does not "fall through" into the list below. |
| Cursor | The **first** claim wins, that is, the deepest element. A parent claims after its children, and without this rule the cursor would always end up being the parent's. |

Among **siblings** that overlap (in a `Stack`, in decorations, in overlays) the order is set by
`Style.ZOrder`: higher means more important. It does not affect drawing order — the higher one is
still drawn on top. Depth works by itself: a child always outranks its ancestor.
