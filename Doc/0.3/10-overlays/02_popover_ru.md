...
|---|---|---|
| `IsOpen` | `bool` | Панель раскрыта? До первого показа — `false`. |
| `Open()` | — | Раскрыть. |
| `Close()` | — | Закрыть. Безопасно звать прямо из колбэка элемента ВНУТРИ панели. |
| `Toggle()` | — | Переключить. |

Намерение применяется на ближайшем кадре, а не мгновенно. Так надо по двум причинам: до первого
кадра хранилища состояния ещё нет, а закрытие из колбэка происходит внутри отрисовки самой панели —
исчезни она посреди неё, разъехались бы клип-команды.

Это тот случай, ради которого метод и появился: клик мимо панели закрывает её сам, а клик по
элементу ВНУТРИ — нет.

```csharp
var pick = Button.Make("Пшеница");
pick.OnClick = () =>
{
    chosen = "Пшеница";
    pop.Close();          // выбрали пункт — панель закрылась
};
```
