...
[Оглавление](../index_ru.md) - Формы | Пред.: [Field](10_field_ru.md) | След.: [Rating](12_rating_ru.md) | [English](11_inputbase_en.md)
...
| `GetText()` | `string` | Текущий текст поля из его состояния — работает и тогда, когда поле в этом кадре **не рисовалось** (скрытая вкладка, свёрнутая панель). До первого показа — `""`. |
| `Flush()` | `bool` | Принудительно закоммитить набранное, не дожидаясь Enter или потери фокуса: игрок набрал значение и сразу нажал «Сохранить». Вызывает `OnChange` (если текст изменился) и `OnCommit`. `false` = поле ещё ни разу не участвовало в кадре. |
...
## Значение, когда поле не на экране

Состояние поля живёт в ID-store и переживает кадры, в которых поле не рисовалось. Поэтому
`GetText()` (и `GetValue()` у `InputNumber`) отдаёт актуальное значение даже со скрытой вкладки
или свёрнутой панели.

Отсюда же следует **обязательное** требование к ключу: состояние находится по `Key`, поэтому
поле, которое пересоздаётся (новый объект на каждом кадре или при каждом открытии окна), обязано
иметь явный `Key` — иначе автоключ сменится вместе с объектом и значение потеряется.

```csharp
var name = new InputText { Key = "colony.name" };

var save = Button.Make("Сохранить");
save.OnClick = () =>
{
    name.Flush();                 // коммит набранного: фокус не терялся, Enter не нажимали
    Settings.ColonyName = name.GetText();
};
```

Разница между колбэками: `OnChange` вызывается при **каждом** изменении текста, `OnCommit` — по
Enter и по потере фокуса. `Flush()` вызывает оба, как если бы игрок нажал Enter.
