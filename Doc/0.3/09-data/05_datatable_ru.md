...
| `TableColumn.TitleContent` | `UiElement` | `null` | Своё содержимое заголовка колонки. Стрелка сортировки и поле фильтра остаются на местах. |
| `MinWeightColumnWidth` | `float` | `90` | Минимальная ширина weight-колонки; не влезают — включается горизонтальный скролл. Сумма фиксированных плюс этот минимум на каждую долю задаёт минимальную ширину содержимого — см. «Точная механика ширины». |
| `ScrollEdgeFade` | `bool` | `true` | Затенение у края, пока вбок есть ещё содержимое. |
...
**Отсюда следует главное, что стоит держать в голове.** Сумма фиксированных ширин плюс
`MinWeightColumnWidth` на каждую долю — это и есть **минимальная ширина содержимого таблицы**.
Досталось меньше — таблица не ужимается, а начинает прокручиваться вбок, и правые колонки вместе
со своими разделителями уезжают за видимую границу.

Выглядит это обманчиво: разделители нарисованы все, но последний оказался за краем, и картинка
читается как «пропала линия сетки». Проверяется просто — прокрутите таблицу вправо.

Пример: четыре колонки по 170 плюс одна доля с `MinWeightColumnWidth = 70` дают 750. Если таблице
досталось 500, три разделителя видны, а четвёртый (на 580) — нет.

Что с этим делать:

* дать таблице нужную ширину либо уменьшить фиксированные;
* снизить `MinWeightColumnWidth`, если доля может быть узкой;
* оставить как есть — прокрутка рабочая, а `ScrollEdgeFade` (включён по умолчанию) затеняет край,
  показывая, что содержимое продолжается.

### Затенение у края

Пока вбок есть ещё содержимое, у соответствующего края тела рисуется мягкое затенение цветом фона
окна. Слева оно появляется только после того, как таблицу прокрутили, — у нетронутого края
показывать нечего.

Отключается флагом `ScrollEdgeFade = false`: полосы прокрутки внизу тогда достаточно.
