...
| `DefaultSortColumn` | `int` | `-1` | Колонка, по которой таблица отсортирована при первом показе; `-1` = без сортировки. Применяется **один раз**, когда заводится состояние: дальше порядок задаёт игрок кликами по заголовкам, и присвоение здесь его не сбросит. Колонка вне диапазона игнорируется. |
...
```csharp
var t = new DataTable(cols) { Key = "log", Style = { AutoHeight = true } };
```
