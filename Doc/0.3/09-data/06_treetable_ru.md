...
| `RowHeight` | `float` | `26` | Высота строки (при `Style.AutoHeight` — минимальная). |
| `DefaultSortColumn` | `int` | `-1` | Колонка, по которой дерево отсортировано при первом показе; `-1` = без сортировки. Применяется **один раз**, когда заводится состояние: как и клик по заголовку, сортирует соседей ВНУТРИ каждого родителя — структура дерева сохраняется; дальше порядок задаёт игрок, и присвоение здесь его не сбросит. Колонка вне диапазона игнорируется. |
| `DefaultSortAsc` | `bool` | `true` | Направление стартовой сортировки. |
| `TableColumn.TitleContent` | `UiElement` | `null` | Своё содержимое заголовка колонки. |
| `TreeTableItem.Content` | `UiElement` | `null` | Своё содержимое ПЕРВОЙ колонки (остальные и так задаются элементами). |
...
По умолчанию строка имеет фиксированную высоту `RowHeight`, и длинный текст в ячейке обрезается.
`Style.AutoHeight = true` заставляет строку подстраиваться под содержимое: текст переносится и
строка растёт настолько, насколько нужно. `RowHeight` при этом работает как минимальная высота.

```csharp
var t = new TreeTable(cols) { Key = "stock", Style = { AutoHeight = true } };
```

Прокрутка остаётся виртуализированной: измеряются все строки, но рисуются только видимые. На
списках в тысячи строк измерение стоит доли миллисекунды при бюджете кадра 16,7 мс.
