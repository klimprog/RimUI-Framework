...
`Style.AutoHeight = true` заставляет строку подстраиваться под содержимое: текст
переносится и строка растёт настолько, насколько нужно, вместо обрезки по фиксированной
