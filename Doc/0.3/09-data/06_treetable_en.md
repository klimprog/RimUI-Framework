...
By default a row has the fixed height `RowHeight`, and long text in a cell is cut off.
`Style.AutoHeight = true` makes the row fit its content: text wraps and the row grows as far as it
needs to. `RowHeight` then acts as a minimum height.

```csharp
var t = new TreeTable(cols) { Key = "stock", Style = { AutoHeight = true } };
```

Scrolling stays virtualized: every row is measured, but only the visible ones are drawn. On lists
of thousands of rows the measuring costs a fraction of a millisecond against a 16.7 ms frame
budget.
