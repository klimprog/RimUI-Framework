...
| `ShowTitle` | `bool` | `true` | Показывать заголовок. `false` — центральная зона шапки остаётся под свои элементы. |
| `ShowClose` | `bool` | `true` | Показывать крестик закрытия. |
| `HeaderLeftBefore` / `HeaderLeftAfter` | `List<UiElement>` (readonly) | пусто | Свои элементы в левой зоне шапки — до и после гамбургера. |
| `HeaderCenterBefore` / `HeaderCenterAfter` | `List<UiElement>` (readonly) | пусто | То же в центральной зоне — до и после заголовка. |
| `HeaderRightBefore` / `HeaderRightAfter` | `List<UiElement>` (readonly) | пусто | То же в правой зоне — до и после крестика. |
...
Шапка делится на три зоны, и в каждую можно добавить свои элементы — до и после штатного
содержимого. Так кнопка встаёт именно там, где нужно, а не «где-нибудь рядом».

```
[ HeaderLeftBefore | гамбургер | HeaderLeftAfter ] … [ HeaderCenterBefore | заголовок | HeaderCenterAfter ] … [ HeaderRightBefore | крестик | HeaderRightAfter ]
```

```csharp
var w = new ModalWindow(body) { Title = "Склад" };

var help = Button.Make("?");
help.OnClick = () => MessageWindow.OfText("Справка", "…").Show();
w.HeaderRightBefore.Add(help);          // слева от крестика

var pin = new ToggleButton("Закрепить");
w.HeaderLeftAfter.Add(pin);             // справа от гамбургера
```

Штатные части отключаются: `ShowTitle = false` освобождает центральную зону целиком (например
под строку поиска), `ShowClose = false` убирает крестик.

Центральная зона выравнивает содержимое по центру окна независимо от того, насколько заполнены
боковые: заголовок не съезжает от того, что слева добавилась кнопка.

**Перетаскивание и кнопки не конфликтуют.** Окно тащится за шапку, но нажатие на кнопку **в**
шапке его не двигает: перетаскивание начинается только если клик не забрал кто-то из элементов
шапки. Курсор над зоной перетаскивания — «перемещение».
