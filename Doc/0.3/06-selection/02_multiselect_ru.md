...
`SelectItem.Content` работает так же, как у `Select` (см. его страницу): композиция вместо
подписи, `Text` остаётся для поиска. Чекбокс пункта остаётся на своём месте.
