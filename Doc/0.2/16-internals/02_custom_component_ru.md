...
var sp = new Spoiler
{
    Key = "settings_advanced",       // стабильный ключ — обязательно, см. ниже
    Title = "Дополнительно",
    Content = body,
...
### Состояние: ID-store и стабильный `Key`
...
**`Key` обязан быть стабильным между кадрами.** Ключ вроде `"sp" + i` в цикле — нормально, если
`i` стабилен. Ключ, зависящий от времени, случайного числа или порядка сортировки — источник
трудноуловимых багов: состояние «теряется», спойлер захлопывается сам, hover не срабатывает.
...
- [ ] Состояние — в ID-store по стабильному `Key`, наружу — методами.
...
**Нестабильный `Key`.** Состояние теряется каждый кадр: скролл прыгает, спойлер закрывается,
`HoverEnter` стреляет без остановки.
